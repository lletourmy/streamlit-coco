# PRD — `tableau_to_semantic`

**Status:** Ships in streamlit-coco **`0.1.6`** (slice 1–6)
**Owner:** Laurent Letourmy
**Kind:** streamlit-coco example app (public, ships in `examples/`)
**Last updated:** 2026-08-13

> An example app that reads Tableau workbooks, shows what is actually inside them,
> and moves the KPI logic and the access rules down into Snowflake — one semantic
> view, one row access policy — then generates a Streamlit app that **must** consume
> that view (live via `SEMANTIC_VIEW(...)`, or disconnected on sample frames).
>
> **Demo pack (default):** `ts_content` + `ts_users` — project-leader branch present
> vs dropped. The full MIT set of four remains under `examples/tableau_legacy/`.
...
1. **The source is unmodelled.** A `.twb` is XML on disk. It is in no semantic view,
   no catalogue, no warehouse. Snowflake Intelligence cannot see it. The only way in
   is an agent with `Read` / `Glob` / `Grep` — which is exactly what CoCo is.
...
- **Not a pixel-perfect Tableau clone.** Screen 6 emits a Streamlit *consumer* of the
...
Drop `.twb` / `.twbx` files, or pick a bundled fixture pack.
...
**Step 2 — Streamlit app for selected Tableau dashboards.** Migrate the chosen
...
- **Run on the fly:** **Preview** (header) opens the same right-hand slot as
  Copilot. The split uses `streamlit_extras.resizable_columns`. **Full width**
  hides the
  wizard and gives the preview the whole row. **Run** starts `streamlit run` on
  `127.0.0.1:8511` (disconnected or live) and embeds it in that panel; **Stop** /
  **Open**. Building with python opens Preview automatically. The pane previews
...
│   ├── streamlit_app_gen.py  Tableau dashboards → Streamlit consumer
...
Fixtures are **not** copied here — the app points at
`examples/tableau_legacy/workbooks/`. One copy, one attribution.
...
| Streamlit preview collides with the wizard port | Generated app launches on **8511**, not 8501. |

...
def _build_artifacts() -> dict[str, str]:
    ...
    return {
        ...
        "row_access_policy.sql": build_row_access_policy_sql(access, all_decisions=all_dec),
    }
...
def _render_coco_builder(
    *,
    ...
    if selected == "BRIEF.md" and refresh:
        st.session_state[body_key] = disk or prompt
    elif not str(st.session_state.get(body_key) or "").strip():
    ...
    if st.button(
        ...
        st.rerun()
    coco_disabled = (not chosen) or (not is_connected()) or (not brief_saved)
    if st.button(
        "Generate with CoCo",
        type="primary",
        ...
        st.rerun()
    if not is_connected():
        st.caption("Connect Copilot first.")
...
def _render_streamlit(artifacts: dict[str, str]) -> None:
    ...
    if not dashboards:
        st.warning("No Tableau dashboards found. Load workbooks on **1 · Load**.")
        return
    ...
    st.caption(f"{len(chosen)} dashboards · {n_sheets} worksheets · `{spec.get('semantic_view')}`")

"""Shared CoCo connection + generation jobs for tableau_to_semantic.

Screens only queue a job; the right rail is Copilot or Preview.
"""
...
def is_copilot_open() -> bool:
    return rail_panel() == "copilot"


def is_preview_open() -> bool:
    return rail_panel() == "preview"


def is_rail_open() -> bool:
    return rail_panel() in {"copilot", "preview"}


def rail_panel() -> str | None:
    value = st.session_state.get(RAIL_KEY)
    if value in {"copilot", "preview"}:
        return value
...
def set_rail(panel: str | None) -> None:
    st.session_state[RAIL_KEY] = panel if panel in {"copilot", "preview"} else None
    st.session_state[OPEN_KEY] = panel == "copilot"
    st.session_state[PREVIEW_KEY] = panel == "preview"
    if panel != "preview":
...
def set_copilot_open(open_: bool) -> None:
    set_rail("copilot" if open_ else None)


def set_preview_open(open_: bool) -> None:
    set_rail("preview" if open_ else None)


def toggle_copilot() -> None:
    set_rail(None if is_copilot_open() else "copilot")


def toggle_preview() -> None:
    set_rail(None if is_preview_open() else "preview")

...
def build_semantic_yaml(
    ...
    *,
    ...
    # Prefer tables touched by multiple workbooks
    ...
    lines: list[str] = [
        ...
        'semantic_model_name: "Tableau Legacy Ops Metrics"',
        "description: >",
        "  Semantic model drafted from Tableau Server ops workbooks.",
        "  Metrics and relationships reconciled via streamlit-coco arbitration.",
        "  Source: TABLEAU_LEGACY.PUBLIC.",
        "",
        "tables:",
    ]
    ...
    for tname in selected:
        ...
        lines.append(f"      Table `{tname}` inferred from Tableau workbook relations.")
        ...
        if tname == selected[0] and metric_decisions:
            ...
            for dec in metric_decisions:
                ...
                lines.append(f"          Tableau formula (reference): {formula}")
                lines.append("        expr: 1")
                lines.append("        data_type: NUMBER")
                lines.append("        default_aggregation: sum")
...
def build_semantic_sql(yaml_body: str, *, decisions: list[dict[str, Any]] | None = None) -> str:
    ...
    return (
        f"-- Provenance decision ids: {provenance}\n"
        f"-- Generated by examples/tableau_to_semantic — review before deploying.\n"
...
def build_row_access_policy_sql(
    ...
    *,
    all_decisions: list[dict[str, Any]] | None = None,
    target_table: str = "historical_events",
) -> str:
    ...
    for dec in kept:
        ...
        # Opinionated mapping: Tableau "project leader" etc. → CURRENT_AVAILABLE_ROLES()
...
-- Generated by examples/tableau_to_semantic — review before deploying.
-- Maps arbitrated Tableau User Filter branches into one Snowflake RAP.

CREATE OR REPLACE ROW ACCESS POLICY {DATABASE}.{SCHEMA}.TTS_USER_FILTER
AS (ID NUMBER) RETURNS BOOLEAN ->
CASE
{body}
END
COMMENT = 'Reconciled from Tableau User Filter calculated fields (streamlit-coco demo)';

ALTER TABLE {DATABASE}.{SCHEMA}.{target_table}
  ADD ROW ACCESS POLICY {DATABASE}.{SCHEMA}.TTS_USER_FILTER ON (id);
"""

"""Session-state accessors and decision provenance for bi_to_semantic."""
...
def hydrate_from_disk() -> list[str]:
    """Load ``out/*.json``, workspace sources, and a saved CoCo brief once per session."""
    flag = _key("hydrated")
    ...
    if list_bi_files(WORKSPACE_DIR):
        set_cwd_ready(True)
...
def has_mapped_estate() -> bool:
    """True when an estate map is in session or saved under ``out/``."""
    estate = get_estate()
    if estate and (estate.get("tables") or estate.get("joins")):
        return True
    from engine.persist import load_payload, step_has_output

    return step_has_output("estate", load_payload("estate"))


def clear_project() -> None:
    """Wipe workspace sources, ``out/`` artifacts, and in-memory step state."""
    import shutil

    from engine.bi_sources import list_bi_files
    from engine.paths import OUT_DIR, WORKSPACE_DIR
    from engine.preview_server import stop_preview
    from engine.streamlit_app_gen import APP_DIRNAME, APP_DIRNAME_COCO

    stop_preview(OUT_DIR / APP_DIRNAME)
    stop_preview(OUT_DIR / APP_DIRNAME_COCO)
    ...
    uploads = WORKSPACE_DIR / "_uploads"
    if uploads.is_dir():
    ...
    st.session_state[_key("estate")] = None
    st.session_state[_key("metrics")] = None
    st.session_state[_key("access")] = None
    st.session_state[_key("decisions")] = []
    st.session_state[_key("cwd_ready")] = False
    st.session_state[_key("generate_prompted")] = False
    st.session_state[_key("hydrated")] = True
    st.session_state[_key("hydrated_files")] = []

"""BI → Semantic adapter around the generic ``st_coco.copilot_rail``."""
...
from engine.access_parse import merge_access_payloads
...
def _apply_structured_output(kind: str, data: dict[str, Any]) -> None:
    ...
    on_ok = setters[kind]
    if kind == "access":
        existing = state.get_access()

        def on_ok(payload: dict[str, Any]) -> None:
            state.set_access(merge_access_payloads(existing, payload))

    ok, errors = accept_structured(data, schema=schema, on_ok=on_ok)
    clear_job()
    if ok:
...
def render_copilot_rail() -> None:
    ...
    def _on_connect(chosen: str) -> None:
        ...
        st.session_state.pop("tts_coco_connect_hint", None)
    ...
    placeholder = "Ask about the BI sources…"
    ...
    if st.session_state.get("tts_coco_connect_hint") and not is_connected():
        st.info(
            "Connect to generate the Streamlit app with CoCo.",
            icon=":material/link:",
        )
    ...
    copilot_rail(
        ...
        connect_caption="CoCo CLI auth — Tableau / Power BI files live in the agent workspace.",

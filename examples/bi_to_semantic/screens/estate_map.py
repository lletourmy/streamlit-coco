"""Screen 2 — Estate map (deterministic Tableau XML / Power BI TMDL → Graphviz)."""
...
def _contested_tables(estate: dict) -> set[str]:
    by_table: dict[str, set[tuple[str, ...]]] = {}
    for row in estate.get("table_sources") or []:
        name = str(row.get("table") or "")
        if not name:
            continue
        cols = tuple(row.get("columns") or [])
        by_table.setdefault(name, set()).add(cols)
    return {name for name, shapes in by_table.items() if len(shapes) > 1}


def _to_dot(estate: dict) -> str:
    shared = _shared_tables(estate)
    contested = _contested_tables(estate)
    lines = [
        "digraph estate {",
        "  rankdir=LR;",
    ...
    for name in sorted(table_names):
        ...
        if name in contested:
            lines.append(
                f'  "{safe}" [fillcolor="#FECACA", color="#B91C1C", penwidth=2, '
                f'label="{safe}\\n(contested)"];'
            )
        elif name in shared:
            lines.append(
...
def _render_estate(estate: dict) -> None:
    ...
    st.markdown(
        ...
        "(same name, different columns — that overlap is what a shared semantic view is for)."
    )
    ...
    with right:
        ...
        if contested:
            st.markdown("##### Contested names")
            st.write(", ".join(f"`{t}`" for t in sorted(contested)))
        elif shared:
        ...
        origins = [
            row
            for row in (estate.get("table_sources") or [])
            if row.get("source") and str(row.get("table")) in (contested or shared)
        ]
        if origins:
            st.markdown("##### M sources (shared / contested)")
            st.dataframe(
                [
                    {
                        "table": r.get("table"),
                        "source file": r.get("workbook"),
                        "M": r.get("source"),
                    }
                    for r in origins
                ],
...
def run() -> None:
    ...
    st.markdown(
        "Map entities, joins, and which source touches what — "
        "**parsed from Tableau `.twb` XML and/or Power BI `.pbix`** (no agent). "
        "Shared names are highlighted; contested names (same label, different columns) in red."
    )
    ...
    if estate:
        ...
        st.caption("Click **Map the estate** to parse the sources.")

"""Screen 3 — KPI inventory (deterministic Tableau fields / Power BI measures)."""
...
def _render_metrics(payload: dict) -> None:
    ...
    st.markdown(
        f"**{len(metrics)}** metrics · **{shared}** in more than one source · "
    ...
    rows = [
        {
            ...
            "sources": ", ".join(m.get("workbooks") or []),
...
def run() -> None:
    ...
    st.markdown(
        "Every calculated field / DAX measure, grouped by name, with conflicts "
        "surfaced — **parsed from Tableau `.twb` XML and/or Power BI TMDL** "
        "(no agent). `plain_english` is a short deterministic note; arbitration "
        "still decides the canonical formula."
    )
    ...
    if payload:
        ...
        st.caption("Click **Inventory KPIs** to parse calculated fields and measures.")

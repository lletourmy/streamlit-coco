...
from engine.bi_sources import partition_sources
...
def _render_access(payload: dict) -> None:
    ...
    for rule in rules:
        ...
        for branch in rule.get("branches") or []:
            ...
            conditions = {
                str(r.get("workbook") or ""): str(b.get("condition") or "")
                for r in rules
                for b in (r.get("branches") or [])
                if _normalize_branch_key(b) == key
            }
            same_shape = len({c for c in conditions.values() if c}) <= 1
            if key in common and same_shape:
                klass = "common"
            elif len(rules) > 1:
                klass = "divergent"
...
def run() -> None:
    ...
    tableau_files, pbi_files = partition_sources(files)
    payload = state.get_access()
    need_coco = bool(tableau_files)
    disabled = need_coco and not is_connected()
    ...
    if pbi_files and not tableau_files:
        st.markdown(
            "Same table names, different models — **`Fact`**, **`Scenario`**, and "
            "**`Date`** appear in both Microsoft samples and agree on none of them. "
            "Public MIT `.pbix` files have no RLS; this screen compares table "
            "contracts (columns + M source) instead. Parsed locally (no agent)."
        )
    elif tableau_files and not pbi_files:
        st.markdown(
        ...
        st.markdown(
            "Tableau User Filters go through Copilot. Power BI compares table "
            "contracts (and RLS if present) locally — public Microsoft samples "
            "have no roles."
        )

    if disabled:
        st.caption(
            "Open **Copilot** → **Connection** → **Connect**, then compare Tableau User Filters."
        )
    elif pbi_files and not tableau_files:
        st.caption("Power BI table contracts are parsed from the .pbix — Copilot is optional.")

    if run_clicked:
        if pbi_files:
            pbi_payload = build_powerbi_access(pbi_files)
            errors = validate_payload(pbi_payload, load_schema("access_rules.schema.json"))
            if errors:
                st.error("Deterministic Power BI extract failed schema validation:")
                for err in errors[:12]:
                    st.caption(f"- {err}")
            else:
                state.set_access(pbi_payload)
        if tableau_files:
            if is_connected():
                queue_access_rules()
            elif not pbi_files:
                st.warning("Connect Copilot to compare Tableau User Filters.")
        st.rerun()
    ...
    if payload:
        st.divider()
        title = (
            "Table contracts — side by side"
            if pbi_files and not tableau_files
            else "Access rules — side by side"
        )
        st.subheader(title)
        _render_access(payload)
    else:
        st.caption("Results appear here after **Compare access rules** finishes.")
